...
export default function EditProfilePage() {
  ...
  const [avatarFile, setAvatarFile] = useState<File | null>(null); // ✅ simpan file asli
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setAvatarFile(file); // ✅ simpan file untuk dikirim ke API
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // ✅ Gunakan FormData agar bisa kirim file gambar + data lain sekaligus
      ...
      const response = await fetch(
        "auth/profile",
        {
          method: "PUT", // atau "PATCH" — sesuaikan dengan API
          credentials: "include",
          headers: {
            // ✅ Jangan set Content-Type saat pakai FormData, browser otomatis set multipart
    ...
    <div className="min-h-screen mx-1 bg-white">
      ...
      <div className="px-6 pt-8 pb-10 max-w-2xl mx-auto mt-6">
        ...
        <div className="flex flex-col gap-5">
          <div>
            <label className="block text-[15px] font-medium text-black mb-2">Store Name</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="block text-[15px] font-medium text-black mb-2">Description</label>
            <textarea
              value={description}
          ...
          <div>
            <label className="block text-[15px] font-medium text-black mb-2">Open Time</label>
            <input
              type="time"
              value={openTime}
          ...
          <div>
            <label className="block text-[15px] font-medium text-black mb-2">Close Time</label>
            <input
              type="time"
              value={closeTime}
        ...
        {/* Action Buttons */}
        <div className="flex items-center justify-end gap-4 mt-10" style={{ maxWidth: "660px" }}>
          <button
            onClick={handleReset}
            disabled={loading}
